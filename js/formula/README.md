...
- round : 반올림한다.(0.5보다 같거나 크면 올리고, 그렇지 않으면 내린다)
- ceil : 무조건 올린다.
- floor : 무조건 내린다.
- abs : 절대값을 돌려준다.
- number : 숫자형으로 변경한다.
...
> ```<input>``` 태그의 name 속성값의 일련번호가 중간에 비는 일이 없도록 주의해야 한다. 
...
#### 2.5.3. 수식이 참조하는 HTML Form 요소를 얻는 방법
...
첫 번째 방법은 일반적이고, 두 번째 방법은 많은 함수를 일정한 규칙하에 추가할 수 있는 방법이다.

첫 번째 방법으로 함수를 추가하기 위해서 참조할 만한 함수는 다음과 같다.
...
### 3.2. HTML Form 요소에 접근하는 방법을 변경하는 방법
