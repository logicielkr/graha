...
//			response.getWriter().append(buffer.toStringBuffer());
	...
	private void post(Record param, HttpServletRequest request, int indent, boolean rdf, Buffer xsl) {
		...
		xsl.appendL(indent, "}");
		xsl.appendL(indent, "if(document.getElementsByClassName(\"msg\")) {");
		xsl.appendL(indent + 1, "for(var i = 0; i &lt; document.getElementsByClassName(\"msg\").length; i++) {");
		xsl.appendL(indent + 2, "if(document.getElementsByClassName(\"msg\")[i].innerText) {");
		xsl.appendL(indent + 3, "alert(_getMessage(document.getElementsByClassName(\"msg\")[i].innerText));");
		xsl.appendL(indent + 2, "} else {");
		xsl.appendL(indent + 3, "var msg = \"\";");
		xsl.appendL(indent + 3, "for(var x = 0; x &lt; document.getElementsByClassName(\"msg\")[i].childNodes.length; i++) {");
		xsl.appendL(indent + 4, "msg += _getMessage(document.getElementsByClassName(\"msg\")[i].childNodes[x].nodeValue);");
		xsl.appendL(indent + 3, "}");
		xsl.appendL(indent + 3, "alert(msg);");
		xsl.appendL(indent + 2, "}");
		xsl.appendL(indent + 1, "}");
		xsl.appendL(indent, "}");
		xsl.appendL(indent, "if(document.getElementById(\"_post\") &amp;&amp; document.getElementById(\"_post\").getElementsByTagName(\"noscript\")) document.getElementById(\"_post\").submit();");
		xsl.appendL(indent, "</script>");
		xsl.appendL(indent, "</body>");
